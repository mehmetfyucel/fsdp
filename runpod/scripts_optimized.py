...
RUN_MODE = os.environ.get("RUN_MODE", "FSDP")
...
FSDP_TRANSFORMER_LAYER_CLS = os.environ.get("FSDP_TRANSFORMER_LAYER_CLS", "Qwen2DecoderLayer")
FSDP_SHARDING_STRATEGY = os.environ.get("FSDP_SHARDING_STRATEGY", "FULL_SHARD")
FSDP_BACKWARD_PREFETCH = os.environ.get("FSDP_BACKWARD_PREFETCH", "BACKWARD_PRE")
FSDP_FORWARD_PREFETCH = os.environ.get("FSDP_FORWARD_PREFETCH", "1") == "1"
FSDP_USE_ORIG_PARAMS = os.environ.get("FSDP_USE_ORIG_PARAMS", "1") == "1"
FSDP_CPU_RAM_EFFICIENT_LOADING = os.environ.get("FSDP_CPU_RAM_EFFICIENT_LOADING", "1") == "1"
FSDP_SYNC_MODULE_STATES = os.environ.get("FSDP_SYNC_MODULE_STATES", "1") == "1"
# "1" => legacy FSDP1'i zorla (torch < 2.6 ile uyumlu). Bos birakilirsa transformers varsayilanini (FSDP2) kullanir.
...
def build_fsdp_config():
    cfg = {
        "fsdp_transformer_layer_cls_to_wrap": FSDP_TRANSFORMER_LAYER_CLS,
    ...
    # FSDP surumunu sabitle (yeni transformers varsayilani FSDP2 -> torch>=2.6 ister).
    if FSDP_VERSION:
        cfg["version"] = int(FSDP_VERSION)
...
def build_training_args():
    base = {
        ...
        "fsdp_config": build_fsdp_config()
    }

    return TrainingArguments(**base)


def inspect_fsdp_model_state(model):
    rank = get_rank()
    total_params = 0
    local_params = 0
    sampled = 0
    ...
    print(
        ...
        f"total_param_numel_before_fsdp_wrap={total_params} "
        f"local_param_numel_before_fsdp_wrap={local_params} "
        f"fsdp_layer_cls={FSDP_TRANSFORMER_LAYER_CLS} "
...
def load_model_for_fsdp():
    ...
    inspect_fsdp_model_state(model)
...
def write_comm_log(all_comm_summaries, train_time_seconds):
    ...
    payload = {
        ...
        "limitation": "FSDP internal NCCL/C++ communication may not be fully captured by this profiler.",
...
def write_results_to_md(metrics):
    ...
    with open(RESULTS_FILE, "a", encoding="utf-8") as f:
        ...
        f.write(f"FSDP Transformer Layer Class: {metrics['fsdp_transformer_layer_cls']}\n")
        f.write(f"FSDP Sharding Strategy: {metrics['fsdp_sharding_strategy']}\n")
        f.write(f"FSDP Backward Prefetch: {metrics['fsdp_backward_prefetch']}\n")
        f.write(f"FSDP Forward Prefetch: {metrics['fsdp_forward_prefetch']}\n")
        f.write(f"FSDP Use Orig Params: {metrics['fsdp_use_orig_params']}\n")
        f.write(f"FSDP CPU RAM Efficient Loading: {metrics['fsdp_cpu_ram_efficient_loading']}\n")
        f.write(f"FSDP Sync Module States: {metrics['fsdp_sync_module_states']}\n")
...
def train():
    ...
    metrics = {
        ...
        "max_length": MAX_LENGTH,
        "world_size": get_world_size(),
        "fsdp_transformer_layer_cls": FSDP_TRANSFORMER_LAYER_CLS,
        "fsdp_sharding_strategy": FSDP_SHARDING_STRATEGY,
        "fsdp_backward_prefetch": FSDP_BACKWARD_PREFETCH,
        "fsdp_forward_prefetch": FSDP_FORWARD_PREFETCH,
        "fsdp_use_orig_params": FSDP_USE_ORIG_PARAMS,
        ...
        "communication_scope": "Python-level torch.distributed only. FSDP internal NCCL/C++ communication may not be fully captured.",
